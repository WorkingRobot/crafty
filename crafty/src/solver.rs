use crate::{
    craft_state::Node, simulator_no_rng::NoRngSimulator, tree::Arena, Action, CraftResult, Input,
    State,
};
...
impl Solver {
    fn from_state(state: State, input: Input, options: SearchOptions) -> Self {
        ...
        SolverBuilder {
            ...
            score_storage_threshold: options
                ...
                .unwrap_or(defaults.score_storage_threshold.unwrap()),
            max_score_weighting_constant: options
                .max_score_weighting_constant
                .unwrap_or(defaults.max_score_weighting_constant.unwrap()),
            exploration_constant: options
                .exploration_constant
                .unwrap_or(defaults.exploration_constant.unwrap()),
            sim_builder: |t: &Arena<State>| {
                NoRngSimulator::new(&mut t.nodes.first_mut().unwrap().state, input)
            },
        }
        .build()
    }

    fn from_context(context: &Input, options: SearchOptions) -> Self {
        Self::from_state(State::new(context), options)
    }
    ...
    fn execute_actions(
        ...
        for action in actions {
            let current_node = &mut self.borrow_tree().get_mut(current_index);
            let current_state = &mut current_node.state;
            ...
            if current_node.available_moves.contains(action) {
    ...
    fn eval(&self, state: &State, parent_state: &State) -> f32 {
    ...
    fn solution(&self) -> (Vec<Action>, State) {
        let mut actions = vec![];
    ...
    pub fn simulate(context: &Input, actions: Vec<Action>) -> (State, Option<CraftResult>) {
...
#[cfg(test)]
mod tests {
    use crate::{Action, CraftOptions, Input, Player, Recipe, SearchOptions, Solver};
    use Action::*;

    fn setup_1() -> (Input, SearchOptions) {
        let recipe = Recipe {
            rlvl: 560,
            level: 90,
            progress_max: 3500,
    ...
    fn setup_2() -> (Input, SearchOptions) {
        let recipe = Recipe {
            rlvl: 580,
            level: 90,
            progress_max: 3900,
    ...
    fn assert_craft(
        ...
        let (end_state, _) = Solver::simulate(context, actions);
    ...
    #[test]
    fn with_buffs_1() {
        let actions = vec![Reflect, Manipulation, PreparatoryTouch, WasteNot2];
        let (context, _) = setup_1();
    ...
    fn trained_finesse_procs() {
        ...
        let (end_state, _) = Solver::simulate(&context, actions);
        // 10 stacks of IQ
        assert_eq!(10, end_state.effects.inner_quiet);
    ...
    fn rotation_should_not_panic_1() {
        ...
        let (context, _) = setup_1();
        Solver::simulate(&context, actions);
    }
    ...
    fn search_should_not_panic() {
        ...
        Solver::search_oneshot(&context, vec![], options);
    }
}
